...
namespace PeterDB {
    ...
    public:
        ...
        */
        ...
        static const size_t NEXT_PAGE_NUM_VAR_SIZE = sizeof(unsigned int);
    ...
    void PageSerializer::writeFreeByteCount(unsigned int freeByteCount, const void *data) {
        byte *writePtr = (byte *) data + PageMetadataConstants::FREEBYTE_COUNT_OFFSET;
        memcpy((void *) writePtr,
               (void *) &freeByteCount,
               PageMetadataConstants::FREEBYTE_COUNT_VAR_SIZE);
    }

    void PageSerializer::writeIsLeafPage(bool isLeafPage, void *data) {
        byte *writePtr = (byte *) data + PageMetadataConstants::IS_LEAF_PAGE_OFFSET;
        memcpy((void *) writePtr,
               (void *) &isLeafPage,
               PageMetadataConstants::IS_LEAF_PAGE_VAR_SIZE);
    }

    void PageSerializer::writeKeyType(const Attribute &keyAttribute, const void *data) {
        byte *writePtr = (byte *) data + PageMetadataConstants::KEY_TYPE_OFFSET;
        memcpy((void *) writePtr,
               (void *) &keyAttribute,
               PageMetadataConstants::KEY_TYPE_VAR_SIZE);
    }

    void PageSerializer::writeNextPageNum(unsigned int nextPageNum, void *data) {
        byte *writePtr = (byte *) data + PageMetadataConstants::NEXT_PAGE_NUM_OFFSET;
        memcpy((void *) writePtr,
               (void *) &nextPageNum,
               PageMetadataConstants::NEXT_PAGE_NUM_VAR_SIZE);
    }

    void PageSerializer::writeNumKeys(unsigned int numKeys, void *data) {
        byte *writePtr = (byte *) data + PageMetadataConstants::NUM_KEYS_OFFSET;
        memcpy((void *) writePtr,
               (void *) &numKeys,
               PageMetadataConstants::NUM_KEYS_VAR_SIZE);
    }
